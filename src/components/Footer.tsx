...
export default function Footer() {
  return (
    <footer className="bg-black border-t border-gray-800 pt-16 pb-8 font-mono">
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 mb-16">
          ...
          <div className="md:col-span-4 flex flex-col items-start font-sans">
            ...
            <p className="text-gray-500 text-[11px] leading-relaxed max-w-xs font-mono font-bold tracking-widest uppercase">
          ...
          {Object.entries(footerLinks).map(([title, links], i) => (
            <div key={title} className={`md:col-span-2 ${i === 0 ? 'md:col-start-7' : ''}`}>
              <h4 className="font-bold text-white mb-6 text-[12px] tracking-widest">[{title}]</h4>
              <ul className="space-y-4 text-[11px] font-bold text-gray-500 tracking-widest">
                {links.map((link) => (
                  <li key={link.label}>
                    <a href={link.href} className="hover:text-cyan-500 hover:pl-2 transition-all before:content-['>'] before:opacity-0 hover:before:opacity-100 before:-ml-3 before:absolute before:text-cyan-500 relative flex items-center">
        ...
        <div className="pt-8 border-t border-gray-800 flex flex-col md:flex-row justify-between items-center gap-6">
          <p className="text-[10px] font-bold tracking-widest text-gray-600 uppercase">
            &copy; {new Date().getFullYear()} GATEFLOW.AI. ALL RIGHTS RESERVED.
          </p>
