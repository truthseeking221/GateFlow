...
import { useModelsWebGL } from '../hooks/useModelsWebGL';
...
export default function ModelsCatalog() {
  ...
  const canvasRef = useModelsWebGL();

  return (
    <section id="models" ref={sectionRef} className="w-full relative bg-black font-sans border-b border-white/10 overflow-hidden">
      <canvas ref={canvasRef} className="absolute inset-0 w-full h-full pointer-events-none opacity-60 z-0 mix-blend-screen" />
      ...
      <div className="max-w-7xl mx-auto pt-24 px-6 pb-24 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-0 relative z-10 border-t border-l border-white/10 shadow-[0_4px_30px_rgba(0,0,0,0.8)]">

          {/* Frontier Models - Left Side */}
          <div className="w-full bg-[#050505] p-8 md:p-12 border-b border-r border-white/10 flex flex-col items-start group transition-colors hover:bg-[#0a0a0a]">
